var app = (function () {
  ...
  var formatDataForD3 = function (data) {
    var initialy = 120;
    var array = [];
    var lastpoint = {x: 0,y: 0};
    array.push([lastpoint.x,lastpoint.y]);
    for (var i = 0; i < data.length; i++) {
      lastpoint.x = i + 1;
      lastpoint.y = (data[i].distance*1);
      array.push([lastpoint.x,lastpoint.y]);
    }
    return array;
  };
  ...
  var renderGraph = function (data) {
    ...
    var initialy = 120, initialx = 320;

    var dataset = formatDataForD3(array);

    var xScale = d3.scaleLinear().rangeRound([0,initialx]);
    var yScale = d3.scaleLinear().rangeRound([initialy,0]);
